...
const ContactPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Artificial Delay: Taaki user ko "Processing" animation dikhe
    const minimumDelay = new Promise(resolve => setTimeout(resolve, 1500));

    try {
      // Dono kaam saath mein honge: Data save + Timer
        ...
        <div className="max-w-400 mx-auto w-full border-x border-dashed border-white/5">
          ...
          <header className="pt-24 md:pt-40 pb-12 md:pb-20 px-6 md:px-12 border-b border-dashed border-white/10 overflow-hidden">
            <motion.h1
              ...
              className="text-[clamp(60px,12vw,180px)] leading-[0.8] uppercase tracking-tighter text-center"
            >
              Contact
            </motion.h1>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 w-full">
            ...
            >
              ...
              {/* Socials Grid with Hover Scale */}
              <motion.div variants={fadeInUp} className="grid grid-cols-2 gap-px bg-white/5 border border-white/10">
                {[
                  ...
                  <motion.div
                    ...
                    key={social.name}
                    className="bg-[#0a0a0a] p-8 md:p-10 transition-colors cursor-pointer group relative overflow-hidden"
                  >
            ...
            <div className="lg:col-span-7 p-6 md:p-12 lg:p-16 bg-[#0d0d0d]/30">
              ...
              >
                ...
                <div className="border-b border-white/10 focus-within:border-white transition-colors pb-4 relative group">
                  ...
                  <div className="relative flex items-center">
                    <select name="category" value={formData.category} onChange={handleChange} required className="bg-transparent w-full outline-none text-lg md:text-xl appearance-none cursor-pointer pr-10 text-white font-light">
                      ...
                      <option value="hello" className="bg-[#0a0a0a]">Other</option>
                    </select>
                ...
                <div className="border-b border-white/10 focus-within:border-white transition-colors pb-4">
                  ...
                  <textarea name="message" rows="5" value={formData.message} onChange={handleChange} required placeholder="Describe your project..." className="bg-transparent w-full outline-none text-lg md:text-xl placeholder:text-neutral-800 resize-none font-light leading-relaxed"></textarea>
                </div>
                ...
                <div className="pt-6 relative">
                  <motion.button
                    ...
                    className="group w-full md:w-auto flex items-center justify-center md:justify-start gap-8 bg-white text-black px-10 py-6 rounded-full font-bold text-[10px] uppercase tracking-[0.4em] hover:bg-neutral-200 transition-all disabled:opacity-50"
                  >
                    <span>{isSubmitting ? "Processing..." : "Send Inquiry"}</span>
                    <motion.div animate={isSubmitting ? { x: [0, 5, 0] } : {}} transition={{ repeat: Infinity }}>
